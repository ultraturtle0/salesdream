        ...
        .done((data) => {
            ...
            for (j=0; j<14; j++) {
                ...
                for (k=0; k<events.length; k++){
                    ...
                    if (moment(date).isSame(moment(event), 'day')) {
                        emptyDay[j] ++;
                        console.log("EMPTYDAY CHECK 1: "+emptyDay[j]);
                        console.log("IT WORKED");
                        timeSections[j] = new Array;
                        var differenceStart = (moment.duration(moment(events[k].start.dateTime).diff(moment("9:30", "HH:mm")))).asMinutes();
                        var differenceEnd = (moment.duration(moment(events[k].end.dateTime).diff(moment("9:30", "HH:mm")))).asMinutes();
                        if(differenceStart > 0){
                            if(emptyDay[j] == 1){
                                console.log("EMPTYDAY CHECK 2: "+emptyDay[j]);
                                console.log("TESTINGTESTING")
                                timeSections[j][0] = new Array (moment("9:30", "HH:mm").format("HH:mm"), moment(events[k].start.dateTime).format("HH:mm"));
                                console.log(timeSections);
                                console.log(timeSections[j]);
                                console.log(timeSections[2]);
                            };
                            if (emptyDay[j] > 1){
                                var diff = (moment.duration(moment(events[k-1].end.dateTime).diff(moment(events[k].start.dateTime)))).asMinutes();
                                if(diff > 0){
                                    console.log("TESTINGTESTINGTESTING")
                                    //timeSections[j][emptyDay[j]-1] = new Array ((moment(events[k-1].end.dateTime)).format("HH:mm"), moment(events[k].start.dateTime).format("HH:mm"));
                                    timeSections[j][emptyDay[j]-1] = new Array(((moment(events[k-1].end.dateTime)).format("HH:mm")), "17:30");
                                    console.log(timeSections[j]);
                                };
                            };
                        /*}if(differenceEnd > 0){
                ...
                if(emptyDay[j] == 0) {
                    timeSections[j] = new Array;
                    timeSections[j][0] = new Array ("9:30", "17:30");
            ...
            for (l=0; l<14; l++) {
                for(m=0; m<timeSections[l].length; m++){
                    ...
                    var difference = (moment.duration((sectionEnd).diff(sectionStart))).asMinutes();
                    console.log(difference);
                    difference = difference - (difference % 30);
                    console.log(difference);
                    sectionNumber = difference / 30;
                    console.log(sectionNumber);
                    buttonSections[l] = new Array;
                    for (n=0; n<sectionNumber; n++){
                        buttonSections[l][n] = new Array;
                        buttonSections[l][n][0]= (moment(sectionStart).add((30*n), 'minutes')).format("h:mm A");
                        console.log(buttonSections[l][n][0]);
                        buttonSections[l][n][1]= (moment(sectionStart).add((30*n), 'minutes')).format("HH:mm");
            ...
            for (i=0; i<14; i++) {
                ...
                for(h=0; h<buttonSections[i].length; h++){
                    $(`#col${i}Date`).append(`
                        <br><button type="button" id="${buttonSections[i][h][1]}" class="btn btn-primary btn-sm">${buttonSections[i][h][0]}</button>
                    `);
                };
            };
